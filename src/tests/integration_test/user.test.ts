...
before(async function () {
  ...
  beforeEach(async function () {
    ...
    const userRepository = AppDataSource.getRepository(User); // Assuming User is your user entity
    await userRepository.query(`TRUNCATE "${userRepository.metadata.tableName}" CASCADE`);

    const roleRepository = AppDataSource.getRepository(Role); // Assuming Role is your role entity
    await roleRepository.query(`TRUNCATE "${roleRepository.metadata.tableName}" CASCADE`);
  ...
  it("should create a new user with role and hashed password", async function () {
    ...
    // Act
    const newUser = await userService.createUser(email, password, roleName, name);
    ...
    assert.strictEqual(newUser.password, "hashedpassword123", "Password should be hashed");
    assert.strictEqual(newUser.roles[0].name, role.name, "User should be assigned a role");
  });
